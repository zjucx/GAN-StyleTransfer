...
class Generator:
  ...
  def __call__(self, input):
    ...
    with tf.variable_scope(self.name):
        ...
        input = tf.pad(inputgen,[[0, 0], [ks, ks], [ks, ks], [0, 0]], "REFLECT")
        o_c1 = self.conv2d(input, ngf, f, f, 1, 1, 0.02,name="c1")
        o_c2 = self.conv2d(o_c1, ngf*2, ks, ks, 2, 2, 0.02,"SAME","c2")
        o_c3 = self.conv2d(o_c2, ngf*4, ks, ks, 2, 2, 0.02,"SAME","c3")

        o_r1 = self.resnet(o_c3, ngf*4, "r1")
        o_r2 = self.resnet(o_r1, ngf*4, "r2")
        o_r3 = self.resnet(o_r2, ngf*4, "r3")
        o_r4 = self.resnet(o_r3, ngf*4, "r4")
        o_r5 = self.resnet(o_r4, ngf*4, "r5")
        o_r6 = self.resnet(o_r5, ngf*4, "r6")

        o_c4 = self.deconv2d(o_r6, [batch_size,64,64,ngf*2], ngf*2, ks, ks, 2, 2, 0.02,"SAME","c4")
        o_c5 = self.deconv2d(o_c4, [batch_size,128,128,ngf], ngf, ks, ks, 2, 2, 0.02,"SAME","c5")
    ...
    def deconv2d(input, outshape, o_d=64, f_h=7, f_w=7, s_h=1, s_w=1, stddev=0.02, padding="VALID", name="deconv2d", do_norm=True, do_relu=True):
        with tf.variable_scope(name):
    ...
    def resnet(input, dim, name="resnet"):
        with tf.variable_scope(name):
